...
class AdverseEventActivity : AppCompatActivity() {
    ...
    private fun addOrEditData(sampleId: Int, cycleNumber: Int, adverseEventId: Int?) {
        ...
        val isServerEvent =
            if (isServerEventStr.isEmpty()) null else if (isServerEventStr == "是") 1 else 0
        val toxicityClassificationStr = parseDefaultContent(tv_toxicity_grading.text.toString())
        val toxicityClassification =
            if (toxicityClassificationStr.isEmpty()) null else getAdverseEventToxicityClassify().indexOf(
                toxicityClassificationStr
            )
        ...
        val medicineRelation =
            if (medicineRelationStr.isEmpty()) null else getAdverseEventMedicineRelation().indexOf(
                medicineRelationStr
            )
        val measureStr = parseDefaultContent(tv_take_measure.text.toString())
        val measure =
            if (measureStr.isEmpty()) null else getAdverseEventMeasure().indexOf(measureStr)
        val isUsingMedicineStr = parseDefaultContent(tv_is_drug_treat.text.toString())
        val isUsingMedicine =
            if (isUsingMedicineStr.isEmpty()) null else if (isUsingMedicineStr == "是") 1 else 0
        ...
        val reportType =
            if (reportTypeStr.isEmpty()) null else getAdverseEventReportType().indexOf(reportTypeStr)
        ...
        val medicineMeasure =
            if (medicineMeasureStr.isEmpty()) null else getAdverseEventMedicineMeasure().indexOf(
                medicineMeasureStr
            )
        val sAERecoverStr = parseDefaultContent(tv_SAE_recover.text.toString())
        val sAERecover =
            if (sAERecoverStr.isEmpty()) null else getAdverseEventSAERecover().indexOf(sAERecoverStr)
        val sAERelationStr = parseDefaultContent(tv_SAE_relation.text.toString())
        val sAERelation =
            if (sAERelationStr.isEmpty()) null else getAdverseEventMedicineRelation().indexOf(
                sAERelationStr
            )
    ...
    private fun loadData(adverseEventBody: AdverseEventBodyBean) {
        ...
        if (adverseEventBody.isServerEvent != null) {
            ...
            ll_server_event.visibility =
                if (adverseEventBody.isServerEvent == 1) View.VISIBLE else View.GONE
        }
        if (adverseEventBody.toxicityClassification != null) {
            tv_toxicity_grading.text =
                getAdverseEventToxicityClassify()[adverseEventBody.toxicityClassification]
        }
        ...
        if (adverseEventBody.medicineRelation != null && adverseEventBody.medicineRelation != -1) {
            tv_drug_relationship.text =
                getAdverseEventMedicineRelation()[adverseEventBody.medicineRelation]
        }
        ...
        if (ll_server_event.visibility == View.VISIBLE) {
            ...
            if (adverseEventBody.sAEState != null) {
                tv_SAE_state.text =
                    if (adverseEventBody.sAEState < 8) getAdverseEventSAEState()[adverseEventBody.sAEState] else adverseEventBody.otherSAEState
            }
            ...
            if (adverseEventBody.medicineMeasure != null) {
                tv_medicine_measure.text =
                    getAdverseEventMedicineMeasure()[adverseEventBody.medicineMeasure]
            }
            ...
            if (adverseEventBody.sAERelation != null) {
                tv_SAE_relation.text =
                    getAdverseEventMedicineRelation()[adverseEventBody.sAERelation]
            }
        }
    }
}
