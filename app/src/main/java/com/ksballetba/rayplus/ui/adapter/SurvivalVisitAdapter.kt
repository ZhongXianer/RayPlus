...
class SurvivalVisitAdapter(layoutResId: Int, data: List<SurvivalVisitListBean.Data>) :
    BaseQuickAdapter<SurvivalVisitListBean.Data, BaseViewHolder>(layoutResId, data) {
    ...
    override fun convert(helper: BaseViewHolder, item: SurvivalVisitListBean.Data?) {
        helper.setText(R.id.tv_visit_date, "随访日期:${item?.interviewTime ?: ""}")
            ...
            .addOnClickListener(R.id.submit_survival_visit_button)
        if (item?.survivalStatus != null)
            helper.setText(R.id.tv_live_status, "生存状态:${getSurvivalStatus()[item.survivalStatus]}")
